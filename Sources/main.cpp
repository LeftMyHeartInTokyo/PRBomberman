...
#define POSITIONCHANGE 1 //zmiana po³o¿enia o 1 kawdrat
...
int main(int argc, char **argv) {
	...
	ResourceMenager RsMng;
 	Player player(2, 3, NULL);//pocz¹tkowa pozycja gracza
	Board board(screen, &player);//za³adowanie planszy 
	...
	if( board.player->isAlive() == false )
	{
		board.DrawSurface( screen, ResourceMenager::images[ ResourceMenager::resIndexes::EKRAN_SMIERCI ], SCREEN_WIDTH/2, SCREEN_HEIGHT/2 );
		SDL_UpdateTexture(scrtex, NULL, screen->pixels, screen->pitch);
		SDL_RenderCopy(renderer, scrtex, NULL, NULL);
		SDL_RenderPresent(renderer);
		Sleep(3000);
	...
	RsMng.~ResourceMenager(); //release img surfeces before SDL quits
